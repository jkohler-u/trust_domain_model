...
# Inspect values of fitting process, here loss function value
model_to_fit.get_fit_result().value()

# plot visualization and save to file
pyddm.plot.plot_fit_diagnostics(model=model_to_fit, sample=samp_simulated)
plt.savefig(PLOT_DIR / "simple-fit.png")
plt.show()

# Solution object sol: access to a number of other useful functions
# # E.g. find probability of response using prob(), such as sol.prob("correct") for the probability of a correct response,
# or the entire histogram of responses using pdf(), such as sol.pdf("error") for the distribution of errors.
